...
def simplify(arr): # Облегчение изображения до чёрно-белого
    new_arr = [] # Создаём список для будущего изображения в RGB формате

    for y in arr:
        row = [] # Создаём список для строки
        for x in y:
            if sum(arr[y][x] / 3 <= 127.5): # Если среднее арифметическое суммы оттенком RGB пикселя меньше 127.5
                row.append((0, 0, 0)) # Заполняем чёрным
            else:
